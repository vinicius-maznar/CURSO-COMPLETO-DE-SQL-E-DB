...
- **1) Traga um relatório geral de TODOS OS CLIENTES (Considerando telefone e endereço);**
	...
	```
	```SQL OUTPUT
	mysql> SELECT C.IDCLIENTE, C.NOME, C.SEXO, C.EMAIL, C.CPF, 
		-> E.LOGRADOURO, E.BAIRRO, E.CIDADE, E.ESTADO, 
		-> T.TIPO, T.NUMERO
	    -> FROM CLIENTE C
	    -> INNER JOIN ENDERECO E ON C.IDCLIENTE = E.ID_CLIENTE
...
- **1) Traga um relatório geral de TODOS OS HOMENS (Considerando telefone e endereço);**
...
- **1) Anote os Ids que possuem campos incorretos;**
  	- IDs com campos incorretos: 12, 13, 18, 19;

	- **PASSO 1:** Dando um `SELECT` nos IDs, utilizando a cláusula `IN (INTERVALO)` para uma prévia confirmação antes da alteração;
